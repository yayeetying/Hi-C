...
def populate(filename, tablename):
    with open(filename) as csv_file: # read csv file given as input
        reader = csv.reader(csv_file, delimiter=',') #separate values by ,
        for row in reader:
            if row[0] != "name" and row[0] != "code": # don't include the info from the headers of csv files
...
populate("students.csv", "roster") #using students file, populate roster table with values
populate("courses.csv", "classes") #using courses file, populate classes table with values
#==========================================================
